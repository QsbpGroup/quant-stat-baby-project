...
def find_volsurge_limitup_days(csv_file_name):
    ...
    for i in range(1, len(volsurge_limitup_days)):
        if volsurge_limitup_days[i] != volsurge_limitup_days[i - 1] + 1:
...
def cal_wave_info(filename, vol_days_index):
    ...
    vol_days = df['TRADE_DT'].values[vol_days_index]
...
if __name__ == '__main__':
    # 调用函数并读取数据
    csv_file_path = '000001.SZ.csv'
    volsurge_limitup_days = find_volsurge_limitup_days(csv_file_path)
    cals = cal_wave_info(csv_file_path, volsurge_limitup_days)
    # print(volsurge_limitup_days)
