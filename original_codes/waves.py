    ...
    if filename.endswith('.csv'):
        ...
        for i in range(min(len(df2),len(df5))):
            ...
            if df2.iloc[0]['TRADE_DT'] < df5.iloc[0]['TRADE_DT']:
                ...
                result_data2.append({'高点日期': high_date, '低点日期': low_date, '最高点价格': high_price,'最低点价格': low_price,'时间间隔（天）': interval, '变化': price_change})
            else:
                interval = (high_date - low_date).days
                price_change = (high_price - low_price) / low_price * 100
                result_data2.append({ '低点日期': low_date,'高点日期': high_date, '最低点价格': low_price,'最高点价格': high_price,'时间间隔（天）': interval, '变化': price_change})
        ...
        for i in range(1, len(df2)):      
            ...
            if df2.iloc[i]['S_DQ_CLOSE'] < df2.iloc[i-1]['S_DQ_CLOSE']:
               #!! 找到当前high点前的low点
                ...
                low_points_before_high=df5[(df5['TRADE_DT']<=t1)&(df5['TRADE_DT']>=t2)]
        ...
        if lower_high_count > 0:
            percentage = "{:.2f}%".format((satisfied_highs_count / lower_high_count) * 100)
        mean_waves_fall = 0
        if waves_fall >0:
        ...
        for i in range(1, len(df5)):      
            ...
            if df5.iloc[i]['S_DQ_CLOSE'] > df5.iloc[i-1]['S_DQ_CLOSE']:
                ...
                high_points_before_low=df2[(df2['TRADE_DT']<=t1)&(df2['TRADE_DT']>=t2)]
        ...
        if higher_low_count > 0:
            percentage_lows = "{:.2f}%".format((satisfied_lows_count / higher_low_count) * 100)
        mean_waves_rise = 0
        if waves_rise >0:
...
waves_result_df.to_csv(waves_output_file_path, index=False, encoding='utf-8-sig')
