...
def hist_plot(df, data_col, hue='', quantile=1, absolute=False):
    """
    ...
    if hue == '':
        sns.histplot(data=quantiled_df, x=data_col, kde=True)
        plt.show()
        return
    if set(df[hue]) == {'rise', 'fall'}:
        sns.histplot(data=quantiled_df, x=data_col, hue=hue, kde=True, palette=['red', 'green'])
    else:
        sns.histplot(data=quantiled_df, x=data_col, hue=hue, kde=True)
    plt.show()
    return
...
def box_plot(df, y, x='', quantile=1, absolute=False):
    ...
    if set(df[x]) == {'rise', 'fall'}:
        ax = sns.boxplot(data=quantiled_df, x=x, y=y, palette=["#EF5C65", "#68AB6A"])
